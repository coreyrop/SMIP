...
class TestLoadLessons(ut.TestCase):
    def runTest(self):
        ...
        def comp_lessons(actual, expected):
            return (
                actual.lesson_title == expected.lesson_title
                and actual.lesson_prompt == expected.lesson_prompt
                and actual.lesson_hint == expected.lesson_hint
                and actual.code_base == expected.code_base
                and actual.lesson_reference == expected.lesson_reference
            )
        ...
        test_lessons = [
            initialize_workbook(
                ...
                registers={
                    j: "".join(choice(digits) for _ in range(10)) for j in range(32)
                },
                references=[],
            )
            for _ in range(5)
