...
def load_lesson_from_workbook(filename):
    ...
    sheet = book.active
    answer = {
        i: int(sheet[get_register_index(i)].value)
        for i in range(32)
        if sheet[get_register_index(i)].value is not None
        and re.match("[+-]?\d", sheet[get_register_index(i)].value) is not None
    }
    ...
    while sheet["G" + str(index)].value is not None:
        references.append(
            {
                "Name": sheet["G" + str(index)].value,
                "Type": sheet["H" + str(index)].value,
                "Path": sheet["I" + str(index)].value,
            }
        )
        index += 1
    return Lesson(
